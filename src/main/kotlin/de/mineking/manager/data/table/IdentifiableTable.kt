...
interface IdentifiableTable<T : Identifiable> : Table<T> {
	...
	fun getAllIds(order: Order? = null): List<String> = manager.driver.withHandleUnchecked { it.createQuery("select id from $name ${(order ?: DEFAULT_ORDER).format()}").mapTo(String::class.java).list() }
	fun getByIds(ids: Collection<Any>, order: Order? = null): List<T> = selectMany(Where.valueContainsField("id", ids), order ?: DEFAULT_ORDER)
